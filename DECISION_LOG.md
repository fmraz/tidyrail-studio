...
- Kept the existing service worker update strategy after a controlled previous-version upgrade test. Immediate activation, client claiming, and old-cache cleanup moved `0.1.7` users to `0.1.8`, preserved their local records, and left the current cache usable offline without adding update prompts or migration code.
